...
- снижение ручной нагрузки на модераторов не менее чем на 50 процентов после пилота;
- сокращение медианного времени реакции на опасный комментарий до 1 минуты;
- снижение доли пропущенного запрещенного контента;
- стабильное и воспроизводимое качество решений;
- получение аналитики о реакции аудитории на публикации.
...
- историческая разметка может быть неполной и несогласованной;
- классы toxic и prohibited_content редкие и требуют повышенной полноты;
- правила запрещенного контента могут меняться;
- пиковая нагрузка может значительно превышать среднесуточную;
- окончательное решение в неоднозначных и юридически значимых случаях принимает модератор.

### 1.3. Что входит в скоуп MVP, а что не входит

В MVP входит:

- прием русскоязычных текстовых комментариев;
- очистка, нормализация, дедупликация и базовое выявление спама;
- многоклассовая классификация комментариев;
- расчет confidence score и маршрутизация по порогам риска;
- очередь модерации и фиксация обратной связи;
- аудит решений, технический мониторинг и мониторинг качества;
- пакетное переобучение модели на проверенной разметке.

В MVP не входит:

- анализ изображений, видео и аудио;
- анализ личных сообщений;
- автоматические санкции к аккаунтам;
- персонализация решения по конкретному пользователю;
- полноценная поддержка языков, кроме русского;
- генерация ответа пользователю с помощью языковой модели.

### 1.4. Предпосылки решения

- Social Network Backend способен публиковать события о новых комментариях.
- Доступны исторические комментарии и решения модераторов минимум за 6 месяцев.
- Эксперты по политике модерации подготовят руководство по разметке.
- У каждого комментария есть стабильный идентификатор, время создания и связь с объектом контента.
- Для пилота доступна группа модераторов, которая проверяет сложные случаи и контрольную выборку.
- Инфраструктура поддерживает Docker, Kubernetes, Kafka, S3-совместимое хранилище и PostgreSQL.

## 2. Методология

### 2.1. Постановка задачи

Основная ML-задача - многоклассовая классификация текста. Для комментария модель возвращает вероятности классов:

- `positive` - выраженная положительная реакция;
- `neutral` - нейтральная реакция без признаков нарушения;
- `negative` - негативная реакция без нарушения правил;
- `toxic` - оскорбления, травля или агрессивная речь;
- `prohibited_content` - контент, запрещенный правилами платформы или законом;
- `spam` - массовая реклама, повторяющиеся или нерелевантные сообщения;
- `need_human_review` - неоднозначный случай, требующий решения модератора.
...
```mermaid
flowchart LR
    A[Новый комментарий] --> B[Очистка и нормализация]
    B --> C[Удаление дублей и признаки спама]
    C --> D[ML-модель]
    D --> E[Класс и confidence score]
    E --> F{Маршрутизация}
    F -->|Безопасный| G[Автопубликация]
    F -->|Сомнительный| H[Очередь модератора]
    F -->|Опасный| I[Скрытие до проверки]
    H --> J[Решение модератора]
    I --> J
    J --> K[Feedback loop]
...
1. Согласовать таксономию классов, руководство по разметке и матрицу действий.
2. Выгрузить исторические комментарии и решения модераторов.
3. Очистить данные, устранить точные и близкие дубли, проверить утечки.
4. Разметить стратифицированную выборку двумя независимыми модераторами.
5. Обучить и оценить baseline-модель.
6. Дообучить transformer-модель, подобрать пороги по классам.
7. Провести offline-проверку, нагрузочное тестирование и проверку безопасности.
8. Запустить shadow mode на 5 процентах потока без влияния на публикацию.
9. Провести пилот с модераторами на 10 процентах потока.
10. Проанализировать метрики, скорректировать пороги и принять решение о rollout.
11. Настроить мониторинг, feedback loop и регулярное переобучение.
...
- `moderation_action` - publish, review, hide или block;
- `is_confirmed` - прошел ли объект экспертную проверку;
- `policy_version` - версия правил модерации;
- `label_source` - ручная разметка, жалоба или историческое решение.
...
1. Сравнить baseline и transformer на фиксированном test.
2. Проверить качество на редких классах и новых временных периодах.
3. Измерить latency и стоимость inference.
4. Провести анализ ошибок с модераторами.
5. Зарегистрировать одобренную модель и пороги в MLflow Model Registry.
...
Пилот проходит в два этапа:

1. Shadow mode на 5 процентах потока в течение 7 дней. Модель формирует решения, но не влияет на публикацию. Решения сравниваются с ручной модерацией.
2. Контролируемый A/B-тест на 10 процентах потока в течение 14 дней. В тестовой группе ML-система автоматически публикует безопасные комментарии и создает приоритетную очередь для остальных. В контрольной группе действует текущий процесс.
...
Пилот успешен при одновременном выполнении условий:

- Recall `prohibited_content` не ниже 0,95;
- Recall `toxic` не ниже 0,90;
- Precision `prohibited_content` не ниже 0,75;
- доля автоматически обработанных комментариев не ниже 50 процентов;
- медианное время реакции на опасный комментарий сокращено минимум на 50 процентов;
- ручная нагрузка снижена минимум на 30 процентов без роста подтвержденных пропусков;
- p95 latency не выше 500 мс;
- доступность сервиса во время пилота не ниже 99,5 процента;
- отсутствуют критические инциденты безопасности и необратимые ошибочные блокировки.

### 3.3. Подготовка пилота

До запуска необходимо:

- утвердить политику классов и пороги маршрутизации;
- подготовить контрольную выборку и назначить модераторов;
- интегрировать ingestion, inference, очередь и интерфейс модератора;
- настроить дашборды качества, нагрузки и ошибок;
- провести нагрузочное тестирование на двукратном ожидаемом пике;
- проверить откат на ручную модерацию;
- провести инструктаж модераторов и службы поддержки;
- назначить владельцев инцидентов и канал оперативной связи;
...
- аутентификация сервисов через короткоживущие учетные данные;
- авторизация пользователей по ролям, принцип минимальных привилегий;
- TLS для сетевого обмена;
- секреты в защищенном хранилище, без хранения в образах и репозитории;
- аудит входов, изменений решений, моделей и порогов;
- сканирование контейнеров и зависимостей;
- ограничение сетевых маршрутов между сервисами;
...
- выполнены критерии успешного пилота;
- бизнес-владелец и владелец политики модерации одобрили пороги;
- модель, датасет, код и параметры имеют версии;
- пройдены функциональные, интеграционные и нагрузочные тесты;
- подтверждены SLA на двукратной пиковой нагрузке;
- проверены fallback, canary rollout и rollback;
- дашборды и алерты покрывают сервисные и ML-метрики;
- проведены проверка безопасности и тест восстановления данных;
- подготовлены runbook, схема эскалации и ответственные;
- модераторы обучены работе с очередью и пересмотром решений;
- настроены аудит, feedback loop и расписание контроля качества.
